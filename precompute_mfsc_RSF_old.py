...
#  IDENTICAL to 6-CH pipeline except:
#       - raw data path
#       - number of channels
# ==============================================================
...
def compute_mfsc(x):
    ...
    for j in range(x.shape[-1]):  # loop channels
    ...
    out = np.concatenate(channel_list, axis=-1)  # (1, 36, 36, C)

    # normalize
    mu, std = out.mean(), out.std() + 1e-9
    out = (out - mu) / std
...
def process_subject(subject_path):
    ...
    for session in os.listdir(subject_path):
        ...
        for file in tqdm(files, desc=f"[RSF MFSC] {subject_id}/{session}"):
            ...
            emg = mat["data"]     # CORRECT KEY

            # ---------------------------------------------------
            # FIXED: Ensure shape is EXACTLY (1, T, 15)
            # ---------------------------------------------------
            if emg.ndim == 2:
                # (T, C) → (1, T, C)
                emg = emg[None, :, :]
